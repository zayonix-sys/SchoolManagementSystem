...
namespace SchoolManagementSystem.Application.Services
{
	public class NoticeService : INoticeService
	{
		...
		private readonly IGenericRepository<Parents> _parentRepository;
		...
		public NoticeService(
			...
			IGenericRepository<Parents> parentRepository,
			...
			try
			{
				...
				if(entity.Method == Method.Email)
				{
					...
					if(entity.RecipientType == RecipientType.General)
					{
						...
						recipients.AddRange(parents.Select(p => p.ParentEmail).Where(email => !string.IsNullOrEmpty(email)).ToList());
					...
					{
						var parents = await _parentRepository.GetAllAsync();
						recipients = parents.Select(p => p.ParentEmail).Where(email => !string.IsNullOrEmpty(email)).ToList();
					}

...
import { getUserPermission, UserPermissionData } from "@/services/userPermissionService";

const UserPermissionListTable = () => {
  const [permissions, setPermissions] = useState<UserPermissionData[]>([]);
  const [filteredPermissions, setFilteredPermissions] = useState<UserPermissionData[]>([]);
  ...
  useEffect(() => {
    const fetchUserPermissionsData = async () => {
      ...
      try {
        ...
        setPermissions(response.data as UserPermissionData[]);
        setFilteredPermissions(response.data as UserPermissionData[]);
      } catch (err) {
        setError(err as any);
      } finally {
        setLoading(false);
